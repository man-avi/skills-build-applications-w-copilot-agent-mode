...
function getHeaders(items) {
  ...
  const first = items[0];
  const preferred = ['id', 'username', 'email', 'first_name', 'last_name', 'date_joined'];
  const keys = [...new Set([
...
function Users() {
  ...
  const headers = getHeaders(filteredUsers.length ? filteredUsers : users);
    ...
    <div className="card shadow-sm mb-4">
      <div className="card-header d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-3">
        <div>
          ...
          <p className="card-text text-muted mb-0">User profiles from the Django REST API.</p>
        </div>
                ...
                {filteredUsers.length > 0 ? (
                  filteredUsers.map((user, index) => (
                    <tr key={user.id || index}>
                      {headers.map((header) => (
                        <td key={header}>{truncate(user[header])}</td>
                      ))}
                    </tr>
                  ))
                ) : (
